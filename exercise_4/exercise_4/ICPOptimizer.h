...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		residuals[2] = T(0);
	   	...
	   	return true;
		*/
		T p[3];  //source point after rigid transform
		T src[3];
		src[0] = T(m_sourcePoint.x());
		src[1] = T(m_sourcePoint.y());
		src[2] = T(m_sourcePoint.z());
		const T* rotation = pose;
		const T* translation = pose + 3;
		ceres::AngleAxisRotatePoint(rotation, src, p);
		p[0] += translation[0];
		p[1] += translation[1];
		p[2] += translation[2];
		T w = ceres::sqrt(T(m_weight));
		residuals[0] = (T(m_targetPoint.x()) - p[0]) * w;
		residuals[1] = (T(m_targetPoint.y()) - p[1]) * w;
		residuals[2] = (T(m_targetPoint.z()) - p[2]) * w;
...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		p[2] += translation[2];
		T w = ceres::sqrt(T(m_weight));
		residuals[0] = w * ((T(m_targetPoint.x()) - p[0]) * T(m_targetNormal[0])
			+ (T(m_targetPoint.y()) - p[1]) * T(m_targetNormal[1])
			+ (T(m_targetPoint.z()) - p[2]) * T(m_targetNormal[2]));
