...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double result = (_x - m_center).dot((_x - m_center)) - (m_radius * m_radius);
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double result = pow((p.dot(p) + (m_radius * m_radius) - (m_a * m_a)), 2) - (4 * (m_radius * m_radius) * ((p(0) * p(0)) + (p(1) * p(1))));
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		//std::cout << "Into Hoppe Eval..." << std::endl;
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		//std::cout << "Into RBF Eval..." << std::endl;
		return result;
	}
